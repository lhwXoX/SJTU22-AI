...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, logvar, noise=True):
        ...
        return mu + epsilon * torch.exp(logvar / 2) if noise else mu
    ...
    # for plot figure
    def input2latent(self, inputs):
        batch_size, channel, height, width = inputs.shape
        assert self.d_input == channel * height * width
        inputs = inputs.view(batch_size, -1)
        mu, logvar = self.encoder(inputs).chunk(2, dim=1)
        latent = self.reparameterize(mu, logvar, noise=False) # remove noise
        return latent
    
    def latent2output(self, latent):
        output = self.decoder(latent).view(-1, 1, 28, 28) # MNIST size: (1, 28, 28)
        return output