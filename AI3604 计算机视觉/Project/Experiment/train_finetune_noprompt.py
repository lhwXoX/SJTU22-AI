...
def train_model(trainloader,valloader,dir_checkpoint,epochs):
    ...
    if args.finetune_type == 'adapter':
        ...
        sam = LoRA_Sam(args,sam,r=1).sam
    sam.to('cuda')
    
    total_num = sum(p.numel() for p in sam.parameters())
    trainable_num = sum(p.numel() for p in sam.parameters() if p.requires_grad)
    ...
    for epoch in pbar:
        ...
        for i,data in enumerate(tqdm(trainloader)):
            ...
            loss =  1.4*loss_dice + 0.6*loss_ce
