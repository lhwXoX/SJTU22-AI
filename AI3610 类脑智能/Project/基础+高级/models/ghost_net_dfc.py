    ...
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True,mode=None,args=None):
        ...
        if self.mode in ['original']:
            ...
            self.short_conv = nn.Sequential( 
                layer.Conv2d(inp, oup, kernel_size, stride, kernel_size//2, bias=False),
                layer.BatchNorm2d(oup),
                layer.Conv2d(oup, oup, kernel_size=(1,5), stride=1, padding=(0,2), groups=oup,bias=False),
                layer.BatchNorm2d(oup),
                layer.Conv2d(oup, oup, kernel_size=(5,1), stride=1, padding=(2,0), groups=oup,bias=False),
                layer.BatchNorm2d(oup),
            ) 
            self.avgp = layer.AvgPool2d(kernel_size=2, stride=2)
...
class GhostBottleneck_dfc(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride, use_se, layer_id=None):
        super(GhostBottleneck_dfc, self).__init__()
        assert stride in [1, 2]

        if layer_id<=1 :
            self.ghost1 = GhostModule_dfc(inp, hidden_dim, relu=True,mode='original')
            self.ghost2 = GhostModule_dfc(hidden_dim, oup, relu=True,mode='original')
        else:
            self.ghost1 = GhostModule_dfc(inp, hidden_dim, relu=True,mode='attn')
            self.ghost2 = GhostModule_dfc(hidden_dim, oup, relu=True,mode='attn')
...
class GhostNetV2(nn.Module):
    def __init__(self, cfgs, num_classes=10, width_mult=1., T=6):
        ...
        block = GhostBottleneck_dfc
        for k, exp_size, c, use_se, s in self.cfgs:
            output_channel = _make_divisible(c * width_mult, 4)
            hidden_channel = _make_divisible(exp_size * width_mult, 4)
            layers.append(block(input_channel, hidden_channel, output_channel, k, s, use_se, layer_id=layer_id))
            input_channel = output_channel
            layer_id += 1
...
#firing rate: 0.03
#FLOPs: 167M
#ANN Energy: 4.6 * 167 = 768.2muj
#SNN Energy: 0.9 * 0.03 * 6 * 167M = 27muj